...
setwd("~/BEDU/Fase2 (R)/sesion8/PostWork8")
...
RawData <- "~/BEDU/Fase2 (R)/sesion8/PostWork8/SP1"
...
# Renombrar columnas
colnames(d1019S);colnames(d1920S)
...
# Ordenamos columnas
data <- select(d1020S, date, home.team, home.score, away.team, away.score:Avg.2.5.U) # Este dataframe tiene todos los datos necesarios
...
#Consideraremos partidos de 15 meses para comenzar a ajustar el modelo. Así, nuestro primer conjunto de entrenamiento consiste
#de datos de partidos hasta el `r ffe` 
...
p <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="purple") + geom_point() +
  labs(x = "Numero de Apuesta", 
       y = "Capital",
       title = "Realizando una secuencia de apuestas maximas") +
...
p <- ggplot(g, aes(x=Num_Ap, y=Capital)) + geom_line( color="purple") + geom_point() +
  labs(x = "Numero de Apuesta", 
       y = "Capital",
       title = "Realizando una secuencia de apuestas promedio") +
  theme(plot.title = element_text(size=12))  +
  theme(axis.text.x = element_text(face = "bold", color="blue" , size = 10, angle = 25, hjust = 1),
        axis.text.y = element_text(face = "bold", color="blue" , size = 10, angle = 25, hjust = 1))  # color, ángulo y estilo de las abcisas y ordenadas 
p
